...
async function verifyOwnChild(studentId, parentId, schoolId) {
...
// Tafuta mwanafunzi (ndani ya shule yako pekee)
router.get('/search-students', async (req, res) => {
  try {
    ...
    let students = await Student.findAll({ where: { schoolId: req.user.schoolId } });
    ...
    res.json(students.map((s) => ({ id: s.id, fullName: s.fullName, className: s.className, linked: !!s.parentId })));
  } catch (err) {
...
router.post('/link-child', async (req, res) => {
  try {
    ...
    if (student.parentId) {
      return res.status(400).json({ error: 'Mwanafunzi huyu tayari ameunganishwa na mzazi mwingine. Wasiliana na Mkuu wa Shule.' });
    }
...
router.get('/children/:studentId/marks', async (req, res) => {
  const check = await verifyOwnChild(req.params.studentId, req.user.id, req.user.schoolId);
...
router.get('/children/:studentId/homework', async (req, res) => {
  const check = await verifyOwnChild(req.params.studentId, req.user.id, req.user.schoolId);
  if (!check.allowed) return res.status(check.status).json({ error: check.error });
  const homework = await Homework.findAll({ where: { className: check.student.className, schoolId: req.user.schoolId } });
  res.json(homework);
});

router.get('/announcements', async (req, res) => {
  const announcements = await Announcement.findAll({
    where: { status: 'approved', schoolId: req.user.schoolId },
...
router.post('/leave-requests', async (req, res) => {
  try {
    ...
    if (!studentId || !type || !date) return res.status(400).json({ error: 'Weka mtoto, aina ya ruhusa, na tarehe.' });

    const check = await verifyOwnChild(studentId, req.user.id, req.user.schoolId);
    if (!check.allowed) return res.status(check.status).json({ error: check.error });

    const leaveRequest = await LeaveRequest.create({
      studentId: Number(studentId), parentId: req.user.id, type, reason, date, status: 'pending',
    });
